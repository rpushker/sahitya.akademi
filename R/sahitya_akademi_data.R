...
#' @param language a character vector of length 1, for exampple, 'Emglish'.
...
#' @return An xml_document containing the html text with author's details like date of birth, etc.
#' @examples
...
#' not present.
#' @author Ravindra Pushker
#' @param author_html a xml_document containing the html text with author's details like date of birth, etc.
#' @return
...
get_authors_dob_dod <- function(language_data) {
  ...
  authors_dob_died <- do.call(rbind, lapply(authors, FUN = function(author) {
    ...
    if (length(author_info) == 0) {
      # Page doesn't exist - For example, Cho.Dharman or infobox block doesn't
      # exist
      get_dob_from_description(author_html)
    } else {
